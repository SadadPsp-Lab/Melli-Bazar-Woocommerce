...
add_action('plugins_loaded', 'init_melli_bazar_woocommerce', 0);

add_filter('woocommerce_currencies', 'mbw_add_currency');

function mbw_add_currency($currencies) {
	$currencies['IRR'] = __('ریال', 'woocommerce');
	$currencies['IRT'] = __('تومان', 'woocommerce');
	return $currencies;
}

add_filter('woocommerce_currency_symbol', 'mbw_add_currency_symbol', 10, 2);

function mbw_add_currency_symbol($currency_symbol, $currency) {
...
function init_melli_bazar_woocommerce() {
	...
	class WC_Gateway_Melli_Bazar extends WC_Payment_Gateway {
		/**
		...
		public function __construct() {
			...
			$this->method_title = __('ملی بازار', 'woocommerce');
			$this->method_description = __('درگاه پرداخت ملی بازار (بانک ملی)! برای استفاده از این درگاه لازم است ماژول curl روی سرور شما فعال باشد.', 'woocommerce');
		...
		function init_form_fields() {
			$this->form_fields = array(
				...
				'enabled' => array(
					...
					'type' => 'checkbox',
					'label' => __('فعالسازی درگاه ملی بازار (بانک ملی)', 'woocommerce'),
					'description' => __('برای فعالسازی درگاه پرداخت ملی بازار (بانک ملی) باید چک باکس را تیک بزنید', 'woocommerce'),
					'default' => 'yes',
					'desc_tip' => true,
				),
				'title' => array(
					...
					'default' => __('ملی بازار (بانک ملی)', 'woocommerce'),
					'desc_tip' => true,
				),
				'description' => array(
		...
		public function admin_options() {
			echo '<h3>' . __('درگاه پرداخت ملی بازار (بانک ملی)', 'wpmaster') . '</h3>';
			echo '<p>' . __('درگاه پرداخت ملی بازار (بانک ملی)') . '</p>';
		...
		function redirect_to_bank($order_id) {
			global $woocommerce;
			$woocommerce->session->order_id_melli_bazar = $order_id;
		...
		function bank_callback() {
			...
			if (isset($_GET['wc_order'])) {
				...
				$order_id = $woocommerce->session->order_id_melli_bazar;
			}
					...
					if (isset($_POST['token']) && isset($_POST['OrderId']) && isset($_POST['ResCode'])) {
						...
						if ($result != false) {
							if ($result->ResCode == 0) {
								...
								update_post_meta($order_id, 'WC_Gateway_Melli_Bazar_OrderId', $OrderId);
								update_post_meta($order_id, 'WC_Gateway_Melli_Bazar_RetrivalRefNo', $RetrivalRefNo);
								update_post_meta($order_id, 'WC_Gateway_Melli_Bazar_TraceNo', $TraceNo);
	 ...
	 **/
	function add_woocommerce_melli_bazar_gateway($methods) {
		$methods[] = 'WC_Gateway_Melli_Bazar';
		return $methods;
	}

	add_filter('woocommerce_payment_gateways', 'add_woocommerce_melli_bazar_gateway');
}
