<?php
/*
Plugin Name: درگاه پرداخت ملی بازار
Plugin URI: https://sadadpsp.ir/
Description: درگاه پرداخت ملی بازار برای افزونه ووکامرس
Version: 1.1
Author: آلماتک
...
function init_melli_woocommerce()
{
    ...
    {
        $methods[] = 'WC_Gateway_Melli_Bazaar';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_woocommerce_melli_gateway');

    class WC_Gateway_Melli_Bazaar extends WC_Payment_Gateway
    {
        /**
        ...
        public function __construct()
        {
            ...
            $this->method_title = __('درگاه ملی بازار', 'woocommerce');
            $this->method_description = __('تنظیمات درگاه پرداخت ملی بازار.', 'woocommerce');
        ...
        function init_form_fields()
        {
            $this->form_fields = array(
                ...
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('فعالسازی درگاه ملی بازار', 'woocommerce'),
                    'description' => __('برای فعالسازی درگاه پرداخت ملی بازار باید چک باکس را تیک بزنید', 'woocommerce'),
                    'default' => 'yes',
                    'desc_tip' => true,
                ),
                'title' => array(
                    ...
                    'default' => __('درگاه ملی بازار', 'woocommerce'),
                    'desc_tip' => true,
                ),
                'description' => array(
                    ...
                    'default' => __('پرداخت امن به وسیله کلیه کارت های عضو شتاب از طریق درگاه ملی بازار', 'woocommerce')
                ),
                ...
                'merchant_id' => array(
                    ...
                    'type' => 'text',
                    'description' => __('شماره پذیرنده درگاه ملی بازار', 'woocommerce'),
                    'default' => '',
                    'desc_tip' => true
                ),
                'terminal_id' => array(
                    ...
                    'type' => 'text',
                    'description' => __('شماره ترمینال درگاه ملی بازار', 'woocommerce'),
                    'default' => '',
                    'desc_tip' => true
                ),
                'terminal_key' => array(
                    ...
                    'type' => 'text',
                    'description' => __('کلید تراکنش درگاه ملی بازار', 'woocommerce'),
                    'default' => '',
                    'desc_tip' => true
                ),
                ...
                'success_massage' => array(
                    ...
                    'type' => 'textarea',
                    'description' => __('متن پیامی که میخواهید بعد از پرداخت موفق به کاربر نمایش دهید را وارد نمایید . همچنین می توانید از شورت کد {transaction_id} برای نمایش کد رهگیری ( کد مرجع تراکنش ) و از شرت کد {SaleOrderId} برای شماره درخواست تراکنش ملی بازار استفاده نمایید .', 'woocommerce'),
                ...
                'failed_massage' => array(
                    ...
                    'type' => 'textarea',
                    'description' => __('متن پیامی که میخواهید بعد از پرداخت ناموفق به کاربر نمایش دهید را وارد نمایید . همچنین می توانید از شورت کد {fault} برای نمایش دلیل خطای رخ داده استفاده نمایید . این دلیل خطا از سایت ملی بازار ارسال میگردد .', 'woocommerce'),
        ...
        function redirect_to_bank($order_id)
        {
            ...
            $orderId = $order_id;
            $callBackUrl = add_query_arg('wc_order', $order_id, WC()->api_request_url('WC_Gateway_Melli_Bazaar'));
            ...
            if ($order_id) {
                ...
                if ($order->status != 'completed') {
                    ...
                    if (isset($_POST['token']) && isset($_POST['OrderId']) && isset($_POST['ResCode'])) {
                        ...
                        $result = $this->sadad_call_api('https://mellibazar.sadadpsp.ir/api/v0/MelliBazarVerify', $parameters);
                        ...
                        if ($result != false) {
                            if ($result->ResCode == 0) {
                                ...
                                update_post_meta($order_id, 'WC_Gateway_Melli_Bazaar_OrderId', $OrderId);
                                update_post_meta($order_id, 'WC_Gateway_Melli_Bazaar_RetrivalRefNo', $RetrivalRefNo);
                                update_post_meta($order_id, 'WC_Gateway_Melli_Bazaar_TraceNo', $TraceNo);
